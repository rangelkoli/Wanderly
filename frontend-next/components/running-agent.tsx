...
import { LANGGRAPH_API_URL, LANGGRAPH_ASSISTANT_ID, UUID_REGEX } from "@/lib/langgraph";
import QuestionCard, {
  type AskHumanAnswersPayload,
  type AskHumanQuestion,
} from "@/components/human-tool-call";
import PlaceSelectionCard, {
  type PlaceOption,
  type PlaceSelectionPayload,
} from "@/components/place-selection-card";
import FlightsResultsCard, { type FlightOption } from "@/components/flights-results-card";
      ...
      <main className="rounded-2xl border border-slate-200 bg-white p-4">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="mt-1 text-sm text-slate-600">
            {activeDay.date_label || "Planned date"} &bull; {activeDay.activities_count ?? 0} Activities &bull; {activeDay.budget_label || "Budget TBD"}
          </p>
        </div>
...
const RunningAgent = () => {
  ...
  const { session: existingSession } = useSession(urlSessionId);
  const { update: updateSessionMut } = useSessionMutations();

  const currentSessionId = useMemo(() => {
    return existingSession?.id ?? urlSessionId;
  }, [existingSession?.id, urlSessionId]);

  const threadId = useMemo(() => {
    return existingSession?.thread_id ?? createdThreadId;
  }, [existingSession?.thread_id, createdThreadId]);
  ...
  const { submit, messages, isLoading, interrupt } = useStream({
    ...
    onThreadId: (newThreadId) => {
      ...
      if (!hasUpdatedSessionRef.current && urlSessionId) {
        ...
        void updateSessionMut(urlSessionId, {
  ...
  React.useEffect(() => {
    ...
    const submitInitialPrompt = async () => {
      try {
        ...
        await updateSessionMut(currentSessionId, { title });
        await submit({
  ...
  }, [currentSessionId, initialPrompt, submit, updateSessionMut]);
